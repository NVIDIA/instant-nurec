...
This package contains the protocol buffer definitions and Python generation tools for the NVIDIA Neural Reconstruction Engine (NRE) gRPC API.

## Contents

- `nre/grpc/protos/`: Protocol buffer definitions (.proto files)
  - `common.proto`: Common data types and structures
  - `sensorsim.proto`: Sensor simulation service definitions
- `nre/grpc/update_generated.py`: Python script to generate Python code from proto files
...
This will generate the following Python files in `nre/grpc/protos/`:
...
The NRE gRPC API provides the following main services:
