...
from libs.pytorch3d_knn.interface import pytorch3d_knn  # type: ignore


KNNReturn = namedtuple("KNNReturn", "dists idx knn")
...
class _knn_points(Function):
    ...
    def forward(  # type: ignore[override]
        ...
        idx, dists = pytorch3d_knn.knn_points_idx(p1, p2, lengths1, lengths2, norm, K, version)
    ...
    def backward(ctx, grad_dists, grad_idx):
        ...
        if not (p2.dtype == torch.float32):
            p2 = p2.float()
        grad_p1, grad_p2 = pytorch3d_knn.knn_points_backward(p1, p2, lengths1, lengths2, idx, norm, grad_dists)
...
def knn_points(
    ...
    p1_dists, p1_idx = _knn_points.apply(p1, p2, lengths1, lengths2, K, version, norm, return_sorted)

    p2_nn = None
    if return_nn:
        p2_nn = knn_gather(p2, p1_idx, lengths2)

    return KNNReturn(dists=p1_dists, idx=p1_idx, knn=p2_nn if return_nn else None)
