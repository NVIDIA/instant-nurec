...
class RayIntersectionTransformFilter(torch.autograd.Function):
    """
    Transforms all timestamped world rays that intersect any time-dependent cuboid to the corresponding ray in the cuboids space
    ...
    - tracks_packinfo: packinfo for the cuboid tracks, N_tracks x 2 [int]
    - tracks_poses_data: pose data of the cuboid tracks, N_poses x 7 [float] in the format of [tx, ty, tz, qx, qy, qz, qw]
    Note that the returned gradient will be in the form of [dt0, dt1, dt2, dr0, dr1, dr2, 0.0], where [dt, dr] is se(3) Lie algebra in tangent space.
    Hence all the non-Euclidean leaves spawning this variable should be lietorch.LieGroupParameter for the optimizer to work properly.

    - tracks_timestamps_us: timestamps of the cuboid tracks, N_poses [int64]
    - cuboids_dims: dimensions of the cuboids, N_cuboids x 3 [float]
    - max_track_n_poses: maximum number of poses for an individual track among all tracks (used within kernels for shared memory allocations) [int]
    - row_major_order: if True, intersection_idxs will be sorted such that the last index changing the fastest (C-style row-major)
                       if False, intersection_idxs will be sorted such that the first index changes the fastest (Fortran-style column-major) [bool]

    Outputs:
    - intersection_rays_cuboid_o: ray origins of cuboid-intersecting rays transformed to the canonical 3d cuboid space of the intersected cuboid, N_intersections x 3 [float]
    - intersection_rays_cuboid_d: normalized ray directions of cuboid-intersecting rays transformed to the canonical 3d cuboid space of the intersected cuboid, N_intersections x 3 [float]
    - intersection_rays_ts: ray start/end distances for each cuboid/ray intersection, N_intersections x 2 [float]
    - intersection_idxs: for each ray-cuboid intersection, the corresponding ray and cuboid indices (sorted lexicographically in C-style row-major or Fortran-style column-major order depending on 'row_major_order' argument) N_intersections x 2 [int]
    """
    ...
    def forward(
        ...
        tracks_packinfo: torch.Tensor,
        tracks_poses_data: torch.Tensor,
        tracks_timestamps_us: torch.Tensor,
        cuboids_dims: torch.Tensor,
        max_track_n_poses: int,
        row_major_order: bool = True,
    ):
        # first pass: perform full transformation of rays if they intersect a cuboid
        (
            intersections_rays_cuboid_o_full,
            intersections_rays_cuboid_d_full,
            intersections_rays_ts_full,
            intersections_rays_pose_idx_full,
            intersections_flags_full,
        ) = vren.ray_cuboidtracks_intersection_transform_filter(
        ...
        if not row_major_order:
            intersections_flags_full = intersections_flags_full.transpose(0, 1).contiguous()
            intersections_rays_cuboid_o_full = intersections_rays_cuboid_o_full.transpose(0, 1).contiguous()
            intersections_rays_cuboid_d_full = intersections_rays_cuboid_d_full.transpose(0, 1).contiguous()
            intersections_rays_ts_full = intersections_rays_ts_full.transpose(0, 1).contiguous()
            intersections_rays_pose_idx_full = intersections_rays_pose_idx_full.transpose(0, 1).contiguous()

        # second pass: filter actual intersection results based on intersection flags and construct compressed outputs
        intersection_idxs = torch.nonzero(intersections_flags_full, as_tuple=False)
        intersection_rays_cuboid_o = intersections_rays_cuboid_o_full[intersection_idxs[:, 0], intersection_idxs[:, 1]]
        intersection_rays_cuboid_d = intersections_rays_cuboid_d_full[intersection_idxs[:, 0], intersection_idxs[:, 1]]
        intersection_rays_ts = intersections_rays_ts_full[intersection_idxs[:, 0], intersection_idxs[:, 1]]
        intersection_rays_pose_idx = intersections_rays_pose_idx_full[intersection_idxs[:, 0], intersection_idxs[:, 1]]

        if not row_major_order:
            intersection_idxs = intersection_idxs.flip(1)

        intersection_idxs = intersection_idxs.int()

        ctx.save_for_backward(
            intersection_rays_cuboid_o,
            intersection_rays_cuboid_d,
            rays_timestamps_us,
            intersection_rays_pose_idx,
            intersection_idxs,
            tracks_poses_data,
            tracks_timestamps_us,
        )

        return intersection_rays_cuboid_o, intersection_rays_cuboid_d, intersection_rays_ts, intersection_idxs

    @staticmethod
    @once_differentiable
    @torch.amp.custom_bwd(device_type="cuda")
    def backward(ctx, dL_drays_cuboid_o, dL_drays_cuboid_d, dL_drays_ts, dL_dintersection_idxs):
        (
            rays_cuboid_o,
            rays_cuboid_d,
            rays_timestamps_us,
        ...
        dL_drays_o, dL_drays_d, dL_dpose_tangent = vren.ray_cuboidtracks_intersection_transform_filter_backward(
            dL_drays_cuboid_o,
            dL_drays_cuboid_d,
            rays_cuboid_o,
            rays_cuboid_d,
        ...
        return dL_drays_o, dL_drays_d, None, None, dL_dpose_tangent, None, None, None, None
...
class CuboidTracks(Tracks):
    ...
    def point_intersection(
        ...
        """
        For each point, returns the mask of the tracks that it is inside

        Inputs:
        - points: 3d world positions, ... x 3 [float]
        - timestamps_us: per point or global timestamp, [...] or [1] [int64]
        - cuboids_dims_padding: if non-None, 3d padding to add to cuboids, broadcastable to N_tracks x 3 [float]
        - return_dense_mask: If true (... x N_tracks) mask is returned else (... x 1) and true if point is inside any cuboidtrack

        Returns:
        - point_cuboidtracks_intersection_mask: for each point, the mask of the tracks that it is inside (or if it is inside any cuboidtrack if return_dense_mask is False)  ... x [1 or N_tracks] [bool]
        """

        data_shape = points.shape[:-1]
        points = points.reshape(-1, 3).contiguous()
        timestamps_us = timestamps_us.reshape(-1).contiguous()
        ...
        mask = vren.point_cuboidtracks_intersection(
            points,
            timestamps_us,
            self.tracks_packinfo,
            self.tracks_poses.data,
        ...
        return mask.reshape(data_shape + (mask.shape[1],))
    ...
    def ray_rolling_shutter_intersection(
        self,
        pixel_idxs: torch.Tensor,
        camera_rays: torch.Tensor,
        camera_poses: torch.Tensor,
        camera_timestamp_start_us: int,
        camera_timestamp_end_us: int,
        w: int,
        h: int,
        shutter_type: int,
        cuboids_dims_padding: torch.Tensor | None = None,
        ...
        """
        Computes the intersection of all cuboid tracks with frame-associated camera rays

        Inputs:
        - pixel_idxs: pixel indices of camera rays (i in [0, width-1] / j in [0, height-1]), N_rays x 2 [int16]
        - camera_rays: normalized 3d directions in camera space, N_rays x 3 [float]
        - camera_poses: start / end pose of camera frame, parameterized as [3d translation / 4d quaternion] 2 x 7 [float]
        - camera_timestamp_start_us: absolute frame start timestamps [int]
        - camera_timestamp_end_us: absolute frame end timestamps [int]
        - w: frame width [int]
        - h: frame height [int]
        - shutter_type: ROLLING_TOP_TO_BOTTOM = 1, ROLLING_LEFT_TO_RIGHT = 2, ROLLING_BOTTOM_TO_TOP = 3,ROLLING_RIGHT_TO_LEFT = 4, GLOBAL = 5 [int]
        - cuboids_dims_padding: if non-None, 3d padding to add to cuboids, broadcastable to N_tracks x 3 [float]
        - max_intersections_per_ray: upper limit of intersections to return [int]
        - with_intersections_ts: if True, intersections_ts will be returned, otherwise only intersections_cnt and intersections_tracks_idx will be returned [bool]
        ...
        intersection_result = vren.ray_cuboidtracks_rolling_shutter_intersection(
            pixel_idxs,
            camera_rays,
            camera_poses,
    ...
    def ray_intersection_transform_filter(
        self,
        rays_o: torch.Tensor,
        rays_d: torch.Tensor,
        rays_timestamps_us: torch.Tensor,
        ...
        """
        Transforms all timestamped world rays that intersect any time-dependent cuboid to the corresponding ray in the cuboid's space.
        ...
        - rays_timestamps_us: per ray timestamp, N_rays [int64]
        - cuboids_dims_padding: if non-None, 3d padding to add to cuboids, broadcastable to N_tracks x 3 [float]
        - row_major_order: if True, intersection_idxs will be sorted such that the last index changes the fastest (C-style row-major)
        if False, intersection_idxs will be sorted such that the first index changes the fastest (Fortran-style column-major) [bool]
    ...
    def visualize(self, frame_step: int = 4, show: bool = True) -> None:
        """
        Visualize the cuboid tracks.

        Inputs:
        - frame_step: only show every 'frame_step' pose for each track [int]
        - show: if True, show the visualization window and block the execution,
        if False, prepare visualization data without blocking and can be used jointly with update to update GUI along with program execution [bool]
        """
        visualizer = get_visualdebugger()
        visualizer.set_properties(up="z_up")

        for track_idx, track_id in enumerate(self.tracks_id):
            pose_start, pose_count = self.tracks_packinfo[track_idx]
            pose_start, pose_count = pose_start.item(), pose_count.item()
            object_dim = self.cuboids_dims[track_idx][None, :].cpu().numpy()

            # Group name based on track ID
            group = f"track-{track_id}"
            group_color = np.random.rand(3)

            for pose_idx in range(pose_start, pose_start + pose_count, frame_step):
                pose = tquat_to_se3_matrix(self.tracks_poses[pose_idx].data).cpu().numpy()
                cuboid_v = np.array(
                    [
                        [-0.5, -0.5, -0.5],
                        [0.5, -0.5, -0.5],
                ...
                node_name = f"t-{track_id}-pose-{pose_idx}"
                # Transform corners by the pose
                cuboid_f = np.array(
                    [[0, 1, 2, 3], [4, 5, 6, 7], [0, 1, 5, 4], [2, 3, 7, 6], [1, 2, 6, 5], [0, 3, 7, 4]]
                )
                mesh_color = group_color * ((pose_idx - pose_start) * 0.5 / pose_count + 0.5)
                visualizer.add_surface_mesh(node_name, cuboid_v, cuboid_f, color=mesh_color, group=group)
