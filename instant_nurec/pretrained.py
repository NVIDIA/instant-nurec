...
``instant_nurec_weights.pt``, downloading from Hugging Face on first use into
...
MODEL_FILENAME = "instant_nurec_weights.pt"


class PretrainedModelError(RuntimeError):
    """Raised when ``instant_nurec_weights.pt`` cannot be resolved."""


def download_instant_nurec_pt(*, cache_dir: Optional[str | Path] = None) -> str:
    """Return the local path to ``instant_nurec_weights.pt``.
