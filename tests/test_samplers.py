...
def _stub_compiled_imports(monkeypatch: pytest.MonkeyPatch):
    ...
    sys.modules.pop("instant_nurec.nre.nrm.datasets.samplers", None)
...
def test_get_closest_frame_index_exact_match_returns_that_index():
    from instant_nurec.nre.nrm.datasets.samplers import get_closest_frame_index
...
def _make_sampler(**overrides):
    from instant_nurec.nre.nrm.config.dataset import AdaptiveSequentialFrameBatchSamplerConfig
    from instant_nurec.nre.nrm.datasets.samplers import AdaptiveSequentialFrameBatchSampler
