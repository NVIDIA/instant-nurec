...
from instant_nurec.utils.model_registry import (
    ModelRegistry,
    ModelRegistryError,
    NgcModelRegistry,
...
def test_log_and_raise_with_format_args(caplog):
    with caplog.at_level(logging.ERROR, logger="instant_nurec.utils.model_registry"):
...
def test_validate_url_rejects_mismatched_domain(tmp_path, caplog):
    reg = _StubRegistry("https://example.com/foo", tmp_path)
    with caplog.at_level(logging.WARNING, logger="instant_nurec.utils.model_registry"):
...
def test_validate_url_returns_false_on_unparseable_url(tmp_path, monkeypatch):
    ...
    import instant_nurec.utils.model_registry as mr
