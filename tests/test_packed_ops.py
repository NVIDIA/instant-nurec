...
def stubbed_module(monkeypatch):
    ...
    monkeypatch.delitem(sys.modules, "instant_nurec._pkg.utils.packed_ops", raising=False)

    import importlib

    mod = importlib.import_module("instant_nurec._pkg.utils.packed_ops")
    return mod, captured
...
def test_values_and_pidx_dataclass_is_frozen_and_slotted():
    ...
    if "instant_nurec._pkg.utils.packed_ops" not in _sys.modules:
        pytest.skip("nre.utils.packed_ops not loaded yet (no prior stub run)")
    mod = _sys.modules["instant_nurec._pkg.utils.packed_ops"]
