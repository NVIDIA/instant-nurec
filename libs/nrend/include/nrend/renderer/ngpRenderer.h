...
class NGPRenderer final : public NRendererImplementation {
    void* m_impl = nullptr;

    void release();

public:
    static constexpr char name[]                           = "ngp-nerf";
    static constexpr ModelVersion::Number minVersionNumber = {0, 0, 0};
    static constexpr ModelVersion::Number maxVersionNumber = {999, 999, 999};

    NGPRenderer(const nlohmann::json& rendererState, const Logger& logger);
    virtual ~NGPRenderer() {
        release();
    }
    ...
    bool supportVersion(const ModelVersion& version,
        ...
        const bool validVersion = (version.is("ingp") || version.is("ingp_nre") || version.is("nerf")) &&
