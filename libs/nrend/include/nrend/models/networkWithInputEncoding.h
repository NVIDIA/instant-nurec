...
template <typename T>
class NetworkWithInputEncoding : public tcnn::Network<float, T> {
public:
    NetworkWithInputEncoding(std::shared_ptr<tcnn::Encoding<T>> encoding, std::shared_ptr<tcnn::Network<T>> network)
        : m_encoding{encoding}, m_network{network} {}

    NetworkWithInputEncoding(
        std::shared_ptr<tcnn::Encoding<T>> encoding,
        ...
        tcnn::json local_network_config       = network;
        local_network_config["n_input_dims"]  = m_encoding->padded_output_width() + n_extra_dims;
        local_network_config["n_output_dims"] = n_output_dims;
        m_network.reset(tcnn::create_network<T>(local_network_config));
    }

    NetworkWithInputEncoding(
        uint32_t n_dims_to_encode, uint32_t n_output_dims, const tcnn::json& encoding, const tcnn::json& network)
        : NetworkWithInputEncoding{
              std::shared_ptr<tcnn::Encoding<T>>{tcnn::create_encoding<T>(n_dims_to_encode, encoding)}, n_output_dims, network} {}

    virtual ~NetworkWithInputEncoding() = default;
    ...
    uint32_t num_encoded_dims() const { return m_encoding->padded_output_width(); }
    ...
    std::unique_ptr<tcnn::Context> forward_impl(
        ...
        forward->network_input = tcnn::GPUMatrixDynamic<T>{
            m_encoding->padded_output_width(), input.n(), stream, m_encoding->preferred_output_layout()};
        forward->encoding_ctx =
            m_encoding->forward(stream, input, &forward->network_input, use_inference_params, prepare_input_gradients);
    ...
    void backward_impl(
        ...
        bool use_inference_params                = false,
        tcnn::GradientMode param_gradients_mode  = tcnn::GradientMode::Overwrite) override {
        tcnn::GPUMatrixDynamic<T> dL_dnetwork_input;
    ...
    size_t n_params() const override { return m_encoding->n_params() + m_network->n_params(); }

    uint32_t padded_output_width() const override { return m_network->padded_output_width(); }

    uint32_t output_width() const override { return m_network->output_width(); }

    uint32_t required_input_alignment() const override {
        return 1; // No alignment required due to encoding
    }

    std::vector<std::pair<uint32_t, uint32_t>> layer_sizes() const override { return m_network->layer_sizes(); }

    uint32_t width(uint32_t layer) const override {
        return layer == 0 ? m_encoding->padded_output_width() : m_network->width(layer - 1);
    }

    uint32_t num_forward_activations() const override { return m_network->num_forward_activations() + 1; }

    std::pair<const T*, tcnn::MatrixLayout> forward_activations(const tcnn::Context& ctx, uint32_t layer) const override {
        const auto& forward = dynamic_cast<const ForwardContext&>(ctx);
        return layer == 0 ? std::make_pair<const T*, tcnn::MatrixLayout>(
                                forward.network_input.data(), m_encoding->preferred_output_layout())
                          : m_network->forward_activations(*forward.network_ctx, layer - 1);
    }

    uint32_t input_width() const override { return m_encoding->input_width(); }

    const std::shared_ptr<tcnn::Encoding<T>>& encoding() const { return m_encoding; }
    ...
    std::string generate_device_function(const std::string& name) const override {
        ...
        preamble << m_network->generate_device_function(network) << "\n\n"
    ...
    std::string generate_backward_device_function(const std::string& name, uint32_t n_threads) const override {
        ...
        std::ostringstream preamble;
        preamble << m_network->generate_backward_device_function(network, n_threads) << "\n\n"
    ...
    uint32_t backward_device_function_shmem_bytes(uint32_t n_threads, tcnn::GradientMode param_gradients_mode) const override {
        return std::max(
            m_network->backward_device_function_shmem_bytes(n_threads, param_gradients_mode),
            m_encoding->backward_device_function_shmem_bytes(n_threads, param_gradients_mode));
    }
