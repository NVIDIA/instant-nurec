...
inline constexpr NGP_HOST_DEVICE uint32_t NERF_GRIDSIZE() {
    return 128;
}
...
inline constexpr NGP_HOST_DEVICE float NERF_RENDERING_NEAR_DISTANCE() {
    return 0.05f;
}
inline constexpr NGP_HOST_DEVICE uint32_t NERF_STEPS() {
    return 1024;
} // finest number of steps per unit length
inline constexpr NGP_HOST_DEVICE uint32_t NERF_CASCADES() {
    return 8;
}

inline constexpr NGP_HOST_DEVICE float SQRT3() {
    return 1.73205080757f;
}
inline constexpr NGP_HOST_DEVICE float STEPSIZE() {
    return (SQRT3() / NERF_STEPS());
} // for nerf raymarch
inline constexpr NGP_HOST_DEVICE float MIN_CONE_STEPSIZE() {
    return STEPSIZE();
}
...
inline constexpr NGP_HOST_DEVICE uint32_t N_MAX_RANDOM_SAMPLES_PER_RAY() {
    return 16;
}

// Any alpha below this is considered "invisible" and is thus culled away.
inline constexpr NGP_HOST_DEVICE float NERF_MIN_OPTICAL_THICKNESS() {
    return 0.01f;
}
...
struct NerfDirection {
    NGP_HOST_DEVICE NerfDirection(const vec3& dir, float dt)
        : d{dir} {}
    vec3 d;
};
