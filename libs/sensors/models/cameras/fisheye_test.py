...
"""Unit tests for OpenCVFisheyeCameraModel.
...
from libs.sensors.models.cameras import CameraModel, OpenCVFisheyeCameraModel


class TestOpenCVFisheyeCameraModel(unittest.TestCase):
    """Tests for OpenCVFisheyeCameraModel."""
    ...
    def setUp(self):
        ...
        # Create a fisheye projection
        self.projection = OpenCVFisheyeProjection.from_components(
            principal_point=torch.tensor([320.0, 240.0], device=self.device, dtype=self.dtype),
            focal_length=torch.tensor([300.0, 300.0], device=self.device, dtype=self.dtype),
            forward_poly=torch.tensor([0.0, 0.0, 0.0, 0.0], device=self.device, dtype=self.dtype),
            resolution=torch.tensor([640.0, 480.0], device=self.device, dtype=self.dtype),
            max_angle=2.0,
            newton_iterations=10,
            min_2d_norm=torch.tensor(1e-6, device=self.device, dtype=self.dtype),
        )

        self.camera = OpenCVFisheyeCameraModel(
    ...
    def test_fisheye_camera_initialization(self):
        """Test fisheye camera model initialization."""
        self.assertEqual(self.camera.resolution, self.resolution)
        self.assertIsInstance(self.camera.projection, OpenCVFisheyeProjection)
        self.assertEqual(self.camera.shutter_type, ShutterType.GLOBAL)

    def test_fisheye_is_camera_model(self):
        """Test that OpenCVFisheyeCameraModel is a CameraModel."""
        self.assertIsInstance(self.camera, CameraModel)

    def test_fisheye_projection_roundtrip(self):
        """Test fisheye projection and back-projection consistency."""
        # Image points within fisheye FOV
        image_points = torch.tensor(
            [[320.0, 240.0], [400.0, 300.0], [200.0, 150.0]], device=self.device, dtype=self.dtype
        )
    ...
    def test_fisheye_transform(self):
        """Test fisheye camera transform."""
        scaled_camera = self.camera.transform(image_domain_scale=0.5)

        self.assertEqual(scaled_camera.resolution, (320, 240))
        self.assertIsInstance(scaled_camera.projection, OpenCVFisheyeProjection)

    def test_fisheye_transform_with_offset(self):
        """Test fisheye camera transform with offset."""
    ...
    def test_camera_rays_normalized(self):
        ...
        image_points = torch.tensor(
            [[320.0, 240.0], [400.0, 300.0], [100.0, 100.0]], device=self.device, dtype=self.dtype
        )
    ...
    def test_camera_rays_to_image_points_with_jacobians(self):
        ...
        self.assertEqual(result.jacobians.shape, (2, 2, 3))
